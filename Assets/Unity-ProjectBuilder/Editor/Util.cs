...
namespace Mobcast.Coffee.Build {
	...
	internal class Util : ScriptableSingleton<Util> {
		protected const string CUSTOM_BUILDER_TEMPLATE = "ProjectBuilderTemplate";
		protected const string EXCLUDE_BUILD_DIR = "__ExcludeBuild";
		...
		public static ProjectBuilder currentBuilder {
			get { return instance.m_CurrentBuilder; }
			private set { instance.m_CurrentBuilder = value; }
		}

		[SerializeField]
		protected ProjectBuilder m_CurrentBuilder;

		/// <summary>On finished compile callback.</summary>
		[SerializeField]
		protected bool m_BuildAndRun = false;

		[SerializeField]
		protected bool m_BuildAssetBundle = false;
		...
		public static void CreateCustomProjectBuilder() {
			...
			string path = EditorUtility.SaveFilePanelInProject("Create Custom Project Builder", "CustomProjectBuilder",
			                                                   "cs", "", "Assets/Editor");
			...
			string templatePath =
				AssetDatabase.GUIDToAssetPath(
					AssetDatabase.FindAssets(CUSTOM_BUILDER_TEMPLATE + " t:TextAsset").First());
